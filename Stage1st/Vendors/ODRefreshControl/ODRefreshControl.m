...
- (id)initInScrollView:(UIScrollView *)scrollView activityIndicatorView:(UIView *)activity
{
    ...
    if (self) {
        ...
        _activity = activity ? activity : [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleMedium];
        ...
        _arrowLayer.fillColor = [[[UIColor darkGrayColor] colorWithAlphaComponent:0.3] CGColor];
...
//        }
    ...
    if (!triggered) {
        ...
        if (self.lastActionTime != nil && [self.lastActionTime timeIntervalSinceNow] > -0.5) {
            CGFloat timeInterval = -[self.lastActionTime timeIntervalSinceNow];
            DDLogDebug(@"[ODRefreshControl] Skip for previous action triggered in %f second before.", timeInterval);
            CGPathRelease(path);
            return;
        }
