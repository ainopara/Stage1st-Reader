...
extension PageRenderer {
    ...
    private func pageData(with floors: [Floor], topic: S1Topic) -> [String: Any] {
        func fontStyleFile() -> String {
            guard let fontSize = AppEnvironment.current.settings.defaults.object(forKey: "FontSize") as? String else {
                return "content_17px.css"
            }

            switch UIDevice.current.userInterfaceIdiom {
            case .phone:
                return "content_\(fontSize).css"
            case .pad:
                return "content_ipad_\(fontSize).css"
            default:
                return "content_17px.css"
            }
        }
        ...
        func colorStyle() -> [String: Any?] {
            ...
            return [
                "background": colorManager.htmlColorStringWithID("5"),
                "text": colorManager.htmlColorStringWithID("21"),
                "border": colorManager.htmlColorStringWithID("14"),
                "borderText": colorManager.htmlColorStringWithID("17"),
            ]
        }
    ...
    func stripTails(content: String) -> String {
        ...
        let s1OrangePattern = brPattern1 + "\\n*—+ ?(来自|來自|from) ?[^<>]*<a href=[^>]*>[^<]*?S1 Orange[^<]*?</a>[^<]*"
        ...
        mutableString.s1_replace(pattern: s1OrangePattern, with: "")
